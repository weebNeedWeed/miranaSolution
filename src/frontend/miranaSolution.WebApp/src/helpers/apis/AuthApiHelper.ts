...
class AuthApiHelper extends BaseApiHelper {
  ...
  async authenticate(
    ...
    const response = await this.init().post<
      ApiResult<
        ...
        | ValidationFailureMessages<AuthenticateUserResponse>
      >
    ...
    if (response.data.status === "fail") {
      const failureMessages = response.data
        .data as ValidationFailureMessages<AuthenticateUserResponse>;
  ...
  async register(request: RegisterUserRequest): Promise<User> {
    const response = await this.init().post<
      ApiResult<User | ValidationFailureMessages<RegisterUserRequest>>
    ...
    if (response.data.status === "fail") {
      const failureMessages = response.data
        .data as ValidationFailureMessages<RegisterUserRequest>;
      throw new Error(Object.values(failureMessages)[0]);
    }
  ...
  async sendRecoveryEmail(request: SendRecoveryEmailRequest): Promise<void> {
    const response = await this.init().post<
      ApiResult<undefined | ValidationFailureMessages<SendRecoveryEmailRequest>>
    ...
    if (response.data.status === "fail") {
      const failureMessages = response.data
        .data as ValidationFailureMessages<SendRecoveryEmailRequest>;
      throw new Error(Object.values(failureMessages)[0]);
    }
  }
  ...
  async redeemToken(request: RedeemTokenRequest): Promise<void> {
    const response = await this.init().post<
      ApiResult<undefined | ValidationFailureMessages<RedeemTokenRequest>>
    >("/api/auth/reset-password/redeem-token", request);
    ...
    if (response.data.status === "fail") {
      const failureMessages = response.data
        .data as ValidationFailureMessages<RedeemTokenRequest>;
      throw new Error(Object.values(failureMessages)[0]);
    }
  }
}
